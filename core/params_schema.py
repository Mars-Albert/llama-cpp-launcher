...
PARAMS = (
    ...
    P(key='main_gpu', tab='gpu', row=5, label='主GPU (--main-gpu):', wattr='adv_main_gpu', widget='spin', default=0, emit='diff', fmt='int', flag='-mg', flags=('-mg', '--main-gpu'), parser='int', min=0, max=MAIN_GPU_MAX),
    P(key='flash_attn', tab='gpu', row=6, label='Flash Attention (--flash-attn):', wattr='adv_flash_attn', widget='combo', default='auto', emit='diff', fmt='str', flag='--flash-attn', flags=('-fa', '--flash-attn'), parser='str', items=["on", "off", "auto"], curtext="auto"),
    P(key='op_offload', tab='gpu', row=7, label='算子卸载 (--op-offload):', wattr='adv_op_offload', widget='check', default=True, emit='bool_neg', flag=['--op-offload', '--no-op-offload'], flags=('--no-op-offload',), parser='bool'),
    P(key='cpu_moe', tab='gpu', row=8, label='CPU MoE (--cpu-moe):', wattr='adv_cpu_moe', widget='check', default=False, emit='bool_pos', flag='--cpu-moe', flags=('--cpu-moe',), parser='bool'),
    P(key='n_cpu_moe', tab='gpu', row=9, label='CPU MoE层数 (--n-cpu-moe):', wattr='adv_n_cpu_moe', widget='spin', default=0, emit='diff_pos', fmt='int', flag='--n-cpu-moe', flags=('-ncmoe', '--n-cpu-moe'), parser='int', min=0, max=999),
    P(key='mmap', tab='gpu', row=10, label='内存映射 (--mmap):', wattr='adv_mmap', widget='check', default=True, emit='bool_neg', flag=['--mmap', '--no-mmap'], flags=('--no-mmap',), parser='bool'),
    P(key='mlock', tab='gpu', row=11, label='内存锁定 (--mlock):', wattr='adv_mlock', widget='check', default=False, emit='bool_pos', flag='--mlock', flags=('--mlock',), parser='bool'),
    P(key='no_host', tab='gpu', row=12, label='无主机内存 (--no-host):', wattr='adv_no_host', widget='check', default=False, emit='bool_pos', flag='--no-host', flags=('--no-host',), parser='bool'),
    P(key='repack', tab='gpu', row=13, label='重打包 (--repack):', wattr='adv_repack', widget='check', default=True, emit='bool_neg', flag=['--repack', '--no-repack'], flags=('-nr', '--no-repack'), parser='bool'),
    P(key='direct_io', tab='gpu', row=14, label='直接IO (--direct-io):', wattr='adv_direct_io', widget='check', default=False, emit='bool_pos', flag='--direct-io', flags=('--direct-io',), parser='bool'),
    P(key='fit', tab='gpu', row=15, label='适配内存 (--fit):', wattr='adv_fit', widget='combo', default='on', emit='diff', fmt='str', flag='--fit', flags=('-fit', '--fit'), parser='str', items=["on", "off"], curtext="on"),
    P(key='fit_target', tab='gpu', row=16, label='适配目标MiB (--fit-target):', wattr='adv_fit_target', widget='text', default='1024', emit='diff', fmt='int', flag='-fitt', flags=('-fitt', '--fit-target'), parser='str', placeholder="1024, 2048, ..."),
    P(key='fit_ctx', tab='gpu', row=17, label='适配最小上下文 (--fit-ctx):', wattr='adv_fit_ctx', widget='spin', default=4096, emit='diff', fmt='int', flag='--fit-ctx', flags=('-fitc', '--fit-ctx'), parser='int', min=256, max=999999),
    P(key='check_tensors', tab='gpu', row=18, label='检查张量 (--check-tensors):', wattr='adv_check_tensors', widget='check', default=False, emit='bool_pos', flag='--check-tensors', flags=('--check-tensors',), parser='bool'),
    P(key='override_tensor', tab='gpu', row=19, label='覆盖张量 (--override-tensor):', wattr='adv_override_tensor', widget='text', default='', emit='diff_nonempty', fmt='str', flag='--override-tensor', flags=('-ot', '--override-tensor'), parser='str', placeholder="tensor_name=type,..."),
    P(key='override_kv', tab='gpu', row=20, label='覆盖KV (--override-kv):', wattr='adv_override_kv', widget='text', default='', emit='diff_nonempty', fmt='str', flag='--override-kv', flags=('--override-kv',), parser='str', placeholder="KEY=TYPE:VALUE,..."),
    P(key='warmup', tab='gpu', row=21, label='预热 (--warmup):', wattr='adv_warmup', widget='check', default=True, emit='bool_neg', flag=['--warmup', '--no-warmup'], flags=('--no-warmup',), parser='bool'),
    P(key='rpc', tab='gpu', row=22, label='RPC服务器 (--rpc):', wattr='adv_rpc', widget='text', default='', emit='diff_nonempty', fmt='str', flag='--rpc', flags=('--rpc',), parser='str', placeholder="host:port,host:port"),
    P(key='threads', tab='gpu', row=23, label='线程数 (--threads):', wattr='adv_threads', widget='spin', default=-1, emit='diff', fmt='int', flag='-t', flags=('-t', '--threads'), parser='int', min=-1, max=256),
    P(key='threads_batch', tab='gpu', row=24, label='批处理线程 (--threads-batch):', wattr='adv_threads_batch', widget='spin', default=-1, emit='diff', fmt='int', flag='-tb', flags=('-tb', '--threads-batch'), parser='int', min=-1, max=256),
    P(key='cpu_mask', tab='gpu', row=25, label='CPU掩码 (--cpu-mask):', wattr='adv_cpu_mask', widget='text', default='', emit='diff_nonempty', fmt='str', flag='--cpu-mask', flags=('-C', '--cpu-mask'), parser='str'),
    P(key='cpu_range', tab='gpu', row=26, label='CPU范围 (--cpu-range):', wattr='adv_cpu_range', widget='text', default='', emit='diff_nonempty', fmt='str', flag='--cpu-range', flags=('-Cr', '--cpu-range'), parser='str', placeholder="lo-hi"),
    P(key='cpu_strict', tab='gpu', row=27, label='严格CPU (--cpu-strict):', wattr='adv_cpu_strict', widget='spin', default=0, emit='diff', fmt='int', flag='--cpu-strict', flags=('--cpu-strict',), parser='int', min=0, max=1),
    P(key='cpu_mask_batch', tab='gpu', row=28, label='批CPU掩码 (--cpu-mask-batch):', wattr='adv_cpu_mask_batch', widget='text', default='', emit='diff_nonempty', fmt='str', flag='--cpu-mask-batch', flags=('-Cb', '--cpu-mask-batch'), parser='str'),
    P(key='cpu_range_batch', tab='gpu', row=29, label='批CPU范围 (--cpu-range-batch):', wattr='adv_cpu_range_batch', widget='text', default='', emit='diff_nonempty', fmt='str', flag='--cpu-range-batch', flags=('-Crb', '--cpu-range-batch'), parser='str', placeholder="lo-hi"),
    P(key='cpu_strict_batch', tab='gpu', row=30, label='批严格CPU (--cpu-strict-batch):', wattr='adv_cpu_strict_batch', widget='spin', default=0, emit='diff', fmt='int', flag='--cpu-strict-batch', flags=('--cpu-strict-batch',), parser='int', min=0, max=1),
    P(key='poll', tab='gpu', row=31, label='轮询级别 (--poll):', wattr='adv_poll', widget='spin', default=50, emit='diff', fmt='int', flag='--poll', flags=('--poll',), parser='int', min=0, max=100),
    P(key='poll_batch', tab='gpu', row=32, label='批轮询 (--poll-batch):', wattr='adv_poll_batch', widget='spin', default=50, emit='diff', fmt='int', flag='--poll-batch', flags=('--poll-batch',), parser='int', min=0, max=100),
    P(key='prio', tab='gpu', row=33, label='优先级 (--prio):', wattr='adv_prio', widget='combo', default='normal', emit='prio', flag='--prio', flags=('--prio',), parser='prio', items=["low", "normal", "medium", "high", "realtime"], curtext="normal"),
    ...
    P(key='numa', tab='gpu', row=34, label='NUMA (--numa):', wattr='adv_numa', widget='combo', default='disable', emit='diff_skip', fmt='str', flag='--numa', skip_values=('disable',), flags=('--numa',), parser='str', items=["disable", "distribute", "isolate", "numactl"]),
