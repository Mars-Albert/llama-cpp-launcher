...
| 基础模式 | 高级模式 |
|---|---|
| 温度、Top-P、Top-K、Min-P、重复惩罚滑块 | 7 个标签页：模型、上下文、采样、GPU、服务、聊天/推理、高级 |
...
实时监控 `llama-server` 的标准输出和错误流，从日志中提取 **40+ 数据点**，按 9 大分类汇总为结构化信息面板。**兼容新旧 llama.cpp 日志格式**（v9174+ 新增 `srv` 前缀格式）。再也不用在一堆终端输出中翻找关键信息。

**捕获的信息类别：**

| 类别 | 详细信息 |
|---|---|
| 🖥️ **GPU** | 设备名称、计算能力、总可用显存、各 GPU 空闲显存 |
...
| 🔧 **系统配置** | CPU 型号/内存、线程配置、OpenMP、Repack |
